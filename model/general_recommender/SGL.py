...
class SGL(AbstractRecommender):
    def __init__(self, sess, dataset, conf):
        ...
        self.ssl_lambda = conf["ssl_lambda"]
    ...
    def build_graph(self):
        ...
        with tf.name_scope("inference"):
            self.ua_embeddings, self.ia_embeddings, self.u_g_embeddings_mutli, self.i_g_embeddings_multi = self._create_lightgcn_SSL_embed()
        ...
        *********************************************************
        ...
        with tf.name_scope("loss"):
            ...
            self.ssl_loss = self.create_ssl_loss()
    ...
    def _create_lightgcn_SSL_embed(self):
        ...
        all_embeddings_multi = tf.stack(all_embeddings, 1)
        all_embeddings = tf.reduce_mean(all_embeddings_multi, axis=1, keepdims=False)
        u_g_embeddings, i_g_embeddings = tf.split(all_embeddings, [self.n_users, self.n_items], 0)

        u_g_embeddings_mutli, i_g_embeddings_multi = tf.split(all_embeddings_multi, [self.n_users, self.n_items], 0)

        return u_g_embeddings, i_g_embeddings, u_g_embeddings_mutli, i_g_embeddings_multi

    def side_ssl_loss(self, x, y):
        normalize_user_emb1 = tf.nn.l2_normalize(x, 1)
        normalize_user_emb2 = tf.nn.l2_normalize(y, 1)
        ...
        pos_score_user = tf.exp(self.ssl_lambda * pos_score_user)
        ttl_score_user = tf.reduce_sum(tf.exp(self.ssl_lambda * ttl_score_user), axis=1)

        ssl_loss_user = -tf.reduce_sum(tf.log(pos_score_user / ttl_score_user))
        return ssl_loss_user

    def create_ssl_loss(self):
        users = tf.nn.embedding_lookup(self.u_g_embeddings_mutli, self.users)
        pos_items = tf.nn.embedding_lookup(self.i_g_embeddings_multi, self.pos_items)

        initial_users, generate_users = users[:,0,:], users[:,1,:]
        initial_items, generate_items = pos_items[:,0,:], pos_items[:,1,:]
        
        ssl_loss_user = self.side_ssl_loss(initial_users, generate_items)
        ssl_loss_item = self.side_ssl_loss(initial_items, generate_users)
